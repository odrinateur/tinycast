...
`windowManagementShowInLauncher` switch: `WindowCommandCatalog` claims an entry first, and
`CustomWindowSize.id(fromEntryID:)` the rest.
...
declines it, which leaves the `AppWindowController` windows. When a key child is in front, such as a
⌘K menu hung off Settings, `WindowTarget` looks one level up through `parent` so the command still
places the window behind it.
...
Settings. A transient panel is `.fullScreenAuxiliary` and stays a no-op, as an unwilling external
window already is.
