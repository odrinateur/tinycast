...
REPO="${REPO:-${GITHUB_REPOSITORY:-odrinateur/tinycast}}"
...

{
    ...
    printf '```sh\nbrew tap odrinateur/tap\nbrew install --cask odrinateur/tap/%s\n```\n' "$CASK"
    printf '%s\n' "Private release assets need \`HOMEBREW_GITHUB_API_TOKEN\` or \`gh auth login\` on the installing Mac."
    # The stable DMG is arm64-only; macOS 26 is the last release that boots on Intel.
    if [ "$CHANNEL" = "stable" ]; then
        printf '%s\n' "On an **Intel** Mac, install \`odrinateur/tap/tinycast-universal\` instead — same app, built with both slices."
    fi
